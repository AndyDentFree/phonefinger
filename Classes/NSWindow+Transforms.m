...
- (NSPoint)screenToWindowCoordinates:(NSPoint)point { // Doesn't work with the transform stuff
    ...
    result.x = point.x - self.frame.origin.x - screenFrame.origin.x;
    result.y = self.frame.origin.y + self.frame.size.height - point.y - screenFrame.origin.y;
    
    a.x = point.x;
    a.y = point.y;
    
    CGAffineTransform original;
    CGSGetWindowTransform(_CGSDefaultConnection(), self.windowNumber, &original);
...
- (void) applyTransform:(CGAffineTransform)transform{
	CGSSetWindowTransform(_CGSDefaultConnection(), self.windowNumber, transform);
}


- (CGAffineTransform) rotationTransformForRadians:(double)radians{
	return [self rotationTransformForPoint:NSMakePoint(self.frame.size.width / 2.0, self.frame.size.height / 2.0) radians:radians];
}
...
- (void)scaleX:(double)x Y:(double)y {
    [self scaleX:x Y:y about:NSMakePoint(self.frame.size.width / 2.0, self.frame.size.height / 2.0)];
}
...
- (void)scaleX:(double)x Y:(double)y about:(NSPoint)point {
    ...
    CGSSetWindowTransform(_CGSDefaultConnection(), self.windowNumber, original);
}

- (void)shearX:(double)x Y:(double)y {
    [self shearX:x Y:y about:NSMakePoint(self.frame.size.width / 2.0, self.frame.size.height / 2.0)];
}
